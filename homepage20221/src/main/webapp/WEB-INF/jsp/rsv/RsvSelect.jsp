		...
		<div id="contents">	
			<div id="bbs_wrap">
				...
				<div class="btn-cont ar">
					<c:choose> <%-- data-값 <- 가상의 속성을 줘서 사용 (클릭이나 다른 옵션을 주고 싶을 때 a태그에 저장할 공간이 없으므로 스크립트에서 data를 제공. -뒤 적은 값을 스크립트에서 불러서 사용 > 주로 쇼핑몰에서 상품을 담을때 상품명을 db에서 가져오지 않고 레이어로 바로 띄워준다) --%>
						<c:when test="${result.applyStatus eq '1'}"><a href="#" class="btn btn-status" data-status="${result.applyStatus}">접수 대기중</a></c:when> <%-- applyStatus를 db에서 가져옴 --%>
...
$(document).ready(function(){
	...
	$(".btn-status").click(function(){
		var status = $(this).data("status"); //위 a태그의 data-status의 값(result.applyStatus)을 status에 저장. .data("변수명") -> a태그나 버튼에서 자주 사용
	...
	//신청가능여부 체크 후 진행 (중복체크 및 신청 가능 여부)
