...
게시물 총 수 : <c:out value="${paginationInfo.totalRecordCount}" />건
	<table>
		...
		<tbody>
			<c:forEach var="result" items="${resultList}">
				<tr>
					<td><c:out value="${result.crud2Id}" /></td>	
								
					<td>
						<c:url var="viewUrl" value="/crud2/select.do">
							<c:param name="crud2Id" value="${result.crud2Id}" />
						</c:url>
						<a href="${viewUrl}"><c:out value="${result.crud2Sj}" /></a>
					</td>

					<td>
						<c:out value="${result.user2Nm}" />
					</td>
					
					<td>
						<c:out value="${result.frst2RegistPnttm}" /> 
					</td>
	...
	<div id="poging_div">
		<ul class="paging_align">
			<c:url var="pageUrl" value="/crud2/selectList.do?" /> <!-- jstl -->
