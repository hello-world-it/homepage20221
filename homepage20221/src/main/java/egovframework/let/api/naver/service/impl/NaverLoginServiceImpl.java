...
public class NaverLoginServiceImpl extends EgovAbstractServiceImpl implements NaverLoginService {
	...
	// 인증 요청문을 구성하는 파라미터 
	private static final String CLIENT_ID = Globals.NAVER_CLIENTID;
	private static final String CLIENT_SECRET = Globals.NAVER_CLIENTSECRET;
	private static final String REDIRECT_URI = Globals.NAVER_REDIRECTURI;
	...
	public String getAuthorizationUrl(HttpSession session, String domain) {
		String redirectUri = "http://" + domain + REDIRECT_URI; //"http://" + domain + "8081" + REDIRECT_URI; //네이버callbackURL -> http://localhost/login/naverLogin.do
	...
	@Override
	public OAuth2AccessToken getAccessToken(HttpSession session, String code, String state, String domain)
			throws IOException {
		...
		if(StringUtils.pathEquals(sessionState, state)) {
			OAuth20Service oauthService = new ServiceBuilder() //토큰 빌더
					.apiKey(CLIENT_ID)
					.apiSecret(CLIENT_SECRET)
					.callback(redirectUri)
					.build(NaverLoginApi.instance());
			
			// Scribe에서 제공하는 AccessToken 획득 기능으로 네아로 Acess Token을 획득
	...
	public String getUserProfile(OAuth2AccessToken oauthToken) throws IOException {
		...
		Request request = new Request.Builder()
				...
				.addHeader("Authorization", "Bearer " + oauthToken.getAccessToken()) //Bearer -> 기본어
				.build(); //newCall
		Response response = client.newCall(request).execute();
		
		return response.body().string(); //json으로 받은걸 문자열로 변환
	}
