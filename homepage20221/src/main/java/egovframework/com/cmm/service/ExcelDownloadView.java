...
public class ExcelDownloadView extends AbstractExcelView {
	...
	protected void buildExcelDocument(Map model, HSSFWorkbook wb, HttpServletRequest req, HttpServletResponse resp) throws Exception {
		...
		Map<String, Object> dataMap = (Map<String, Object>)model.get("dataMap"); //컨트롤러에서 "dataMap" 받아롬
		//dataMap을  타이틀, 컬럼, 밸류로 나눔
		...
		HSSFSheet sheet = wb.createSheet(title); //엑셀 시트 명
		
		if(columMap != null && columMap.size()>0) {
			//header정보 등록
			for(int i=0; i<columMap.size(); i++) { //for문으로 컴럼명 등록
			...
			for(int i=0; i<valueMap.size(); i++) {
				...
				for(int di=0; di<columMap.size(); di++) {
					...
					cell = getCell(sheet, 1+i, di); //곧 사라질 태그
					if(obj instanceof String) { //셀 서식 정의
